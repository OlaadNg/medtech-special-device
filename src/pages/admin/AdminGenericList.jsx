            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-700 text-slate-400 text-xs uppercase tracking-wide">
                  <th className="text-left px-3 sm:px-5 py-3">Title</th>
                  {secondaryField && <th className="text-left px-3 sm:px-5 py-3 hidden md:table-cell">Details</th>}
                  <th className="text-left px-3 sm:px-5 py-3 hidden lg:table-cell">Status</th>
                  <th className="text-left px-3 sm:px-5 py-3 hidden lg:table-cell">Created</th>
              ...
              <tbody className="divide-y divide-slate-700/50">
                {filtered.map(item => (
                  <tr key={item.id} className="hover:bg-slate-700/30 transition-colors">
                    <td className="px-3 sm:px-5 py-3 text-white font-medium max-w-[140px] sm:max-w-xs truncate">{item[displayField] || '(untitled)'}</td>
                    {secondaryField && <td className="px-3 sm:px-5 py-3 text-slate-400 hidden md:table-cell max-w-xs truncate">{item[secondaryField]}</td>}
                    ...
                    <td className="px-3 sm:px-5 py-3 text-slate-500 text-xs hidden lg:table-cell">
                ...
                {filtered.length === 0 && (
                  <tr><td colSpan={5} className="px-3 sm:px-5 py-8 text-center text-slate-500">No records found.</td></tr>
                )}
              </tbody>
            </table>
          </div>
